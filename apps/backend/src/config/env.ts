...
export const env = {
  ...
  QUEUE_REDIS_URL: optionalEnv('QUEUE_REDIS_URL'),
  ...
  // Rate limiting (defaults preserve prior hardcoded behavior; raise only in
  // perf/load-test environments — never in production).
  RATE_LIMIT_GENERAL_MAX: parseIntEnv('RATE_LIMIT_GENERAL_MAX', 200),
  RATE_LIMIT_GENERAL_WINDOW_MS: parseIntEnv('RATE_LIMIT_GENERAL_WINDOW_MS', 15 * 60 * 1000),
  RATE_LIMIT_AUTH_MAX: parseIntEnv('RATE_LIMIT_AUTH_MAX', 20),
  RATE_LIMIT_AUTH_WINDOW_MS: parseIntEnv('RATE_LIMIT_AUTH_WINDOW_MS', 15 * 60 * 1000),
  ...
  // Queue / worker configuration (BullMQ)
  WEBHOOK_QUEUE_CONCURRENCY: parseIntEnv('WEBHOOK_QUEUE_CONCURRENCY', 5),
  WEBHOOK_QUEUE_ATTEMPTS: parseIntEnv('WEBHOOK_QUEUE_ATTEMPTS', 5),
  WEBHOOK_QUEUE_BACKOFF_MS: parseIntEnv('WEBHOOK_QUEUE_BACKOFF_MS', 5_000),
  ANALYTICS_PERSIST_QUEUE_CONCURRENCY: parseIntEnv('ANALYTICS_PERSIST_QUEUE_CONCURRENCY', 1),
  ANALYTICS_PERSIST_QUEUE_ATTEMPTS: parseIntEnv('ANALYTICS_PERSIST_QUEUE_ATTEMPTS', 3),
  ANALYTICS_PERSIST_QUEUE_BACKOFF_MS: parseIntEnv('ANALYTICS_PERSIST_QUEUE_BACKOFF_MS', 2_000),
  ANALYTICS_PERSIST_TICK_INTERVAL_MS: parseIntEnv('ANALYTICS_PERSIST_TICK_INTERVAL_MS', 2_000),
  ANALYTICS_RETENTION_QUEUE_CONCURRENCY: parseIntEnv('ANALYTICS_RETENTION_QUEUE_CONCURRENCY', 1),
  ANALYTICS_RETENTION_QUEUE_ATTEMPTS: parseIntEnv('ANALYTICS_RETENTION_QUEUE_ATTEMPTS', 3),
  ANALYTICS_RETENTION_QUEUE_BACKOFF_MS: parseIntEnv('ANALYTICS_RETENTION_QUEUE_BACKOFF_MS', 30_000),
  ANALYTICS_RETENTION_SCHEDULE_MS: parseIntEnv('ANALYTICS_RETENTION_SCHEDULE_MS', 6 * 60 * 60 * 1000),
...
for (const [key, value] of Object.entries({
  WEBHOOK_QUEUE_CONCURRENCY: env.WEBHOOK_QUEUE_CONCURRENCY,
  WEBHOOK_QUEUE_ATTEMPTS: env.WEBHOOK_QUEUE_ATTEMPTS,
  ANALYTICS_PERSIST_QUEUE_CONCURRENCY: env.ANALYTICS_PERSIST_QUEUE_CONCURRENCY,
  ANALYTICS_PERSIST_QUEUE_ATTEMPTS: env.ANALYTICS_PERSIST_QUEUE_ATTEMPTS,
  ANALYTICS_RETENTION_QUEUE_CONCURRENCY: env.ANALYTICS_RETENTION_QUEUE_CONCURRENCY,
  ANALYTICS_RETENTION_QUEUE_ATTEMPTS: env.ANALYTICS_RETENTION_QUEUE_ATTEMPTS,
  RATE_LIMIT_GENERAL_MAX: env.RATE_LIMIT_GENERAL_MAX,
  RATE_LIMIT_GENERAL_WINDOW_MS: env.RATE_LIMIT_GENERAL_WINDOW_MS,
  RATE_LIMIT_AUTH_MAX: env.RATE_LIMIT_AUTH_MAX,
  RATE_LIMIT_AUTH_WINDOW_MS: env.RATE_LIMIT_AUTH_WINDOW_MS,
})) {
  if (value < 1) {
    throw new Error(`Environment variable ${key} must be at least 1`);
  }
}

for (const [key, value] of Object.entries({
  WEBHOOK_QUEUE_BACKOFF_MS: env.WEBHOOK_QUEUE_BACKOFF_MS,
  ANALYTICS_PERSIST_QUEUE_BACKOFF_MS: env.ANALYTICS_PERSIST_QUEUE_BACKOFF_MS,
  ANALYTICS_PERSIST_TICK_INTERVAL_MS: env.ANALYTICS_PERSIST_TICK_INTERVAL_MS,
  ANALYTICS_RETENTION_QUEUE_BACKOFF_MS: env.ANALYTICS_RETENTION_QUEUE_BACKOFF_MS,
  ANALYTICS_RETENTION_SCHEDULE_MS: env.ANALYTICS_RETENTION_SCHEDULE_MS,
  DLQ_RETENTION_COUNT: env.DLQ_RETENTION_COUNT,
  WORKER_READY_STALE_THRESHOLD_MS: env.WORKER_READY_STALE_THRESHOLD_MS,
})) {
  if (value < 0) {
