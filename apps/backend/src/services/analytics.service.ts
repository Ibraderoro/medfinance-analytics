...
export class AnalyticsService {
  ...
  async enforceRetention(organizationId?: string): Promise<void> {
    ...
    await query("INSERT INTO api_request_metrics_archive SELECT * FROM api_request_metrics WHERE organization_id = $1 AND created_at < NOW() - INTERVAL '90 days' ON CONFLICT DO NOTHING", [organizationId]);
