...
export class AnalyticsService {
  ...
  private async enforceRetentionForOrganization(organizationId: string): Promise<void> {
    await query("SELECT set_config('app.current_tenant_id', $1, true)", [organizationId]);
    await query("SELECT ensure_api_metrics_archive_partition(date_trunc('month', NOW() - INTERVAL '90 days')::date)");
    await query("INSERT INTO api_request_metrics_archive SELECT * FROM api_request_metrics WHERE organization_id = $1 AND created_at < NOW() - INTERVAL '90 days' ON CONFLICT (redis_entry_id, created_at) DO NOTHING", [organizationId]);
    await query("DELETE FROM api_request_metrics WHERE organization_id = $1 AND created_at < NOW() - INTERVAL '90 days'", [organizationId]);
  }
    ...
    try {
      if (!organizationId) {
        ...
        for (const row of orgRows) {
          await this.enforceRetentionForOrganization(row.organization_id);
        }
        return;
      }

      await this.enforceRetentionForOrganization(organizationId);
    } finally {
