...
export function LoginPage() {
  ...
  const fieldErrors = useMemo(
    () => ({
      ...
      organizationId: uuidPattern.test(organizationId.trim())
        ? null
        : 'Organization ID must be a valid UUID.',

      password:
        pendingMfaToken || password.length >= 8
          ? null
      ...
      mfaCode:
        ...
        /^[A-Fa-f0-9]{8}-?[A-Fa-f0-9]{8}$/.test(mfaCode.trim())
          ? null
          : 'Enter the 6-digit verification code or recovery code.',
    }),
  ...
  const startSso = async () => {
    ...
    try {
      ...
      setError(
        'SSO is initiated. Continue with your identity provider using the returned state.',
      );
    } catch (err: unknown) {
      setError(
        isAxiosError(err)
          ? err.response?.data?.error?.message ?? 'Unable to start SSO.'
          : 'Unable to start SSO.',
      );
    } finally {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        setError(
          'Signed in, but we could not load your session. Please refresh the page.',
        );
      }
    } catch (err: unknown) {
  ...
  return (
    ...
    >
      ...
      <p style={{ marginTop: 0, color: '#4b5563' }}>
        Use your MedFinance credentials to access dashboards.
      </p>
      ...
      >
        <label htmlFor="email">
          ...
          <input
            ...
            maxLength={254}
            autoComplete="email"
            disabled={Boolean(pendingMfaToken)}
            style={{
              width: '100%',
              padding: '0.65rem',
              marginTop: 4,
          ...
          {email && fieldErrors.email && (
            <small className={styles.error}>{fieldErrors.email}</small>
          )}
        </label>
        ...
        <label htmlFor="organizationId">
          ...
          {organizationId && fieldErrors.organizationId && (
        ...
        {pendingMfaToken && (
          <label htmlFor="mfaCode">
            ...
            <input
              ...
              type="text"
              inputMode="text"
              pattern="([0-9]{6}|[A-Fa-f0-9]{8}-?[A-Fa-f0-9]{8})"
              value={mfaCode}
              onChange={(e) =>
                setMfaCode(
                  e.target.value
        ...
        {!pendingMfaToken && (
          <button
            type="button"
            disabled={
              Boolean(fieldErrors.email || fieldErrors.organizationId) ||
              isSubmitting
            }
            onClick={startSso}
            style={{
