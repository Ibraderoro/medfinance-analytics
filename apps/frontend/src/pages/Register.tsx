  ...
  return (
    <div className={styles.page} style={{ maxWidth: 420, margin: '4rem auto' }}>
      ...
      <form onSubmit={submit} style={{ display: 'grid', gap: 12 }} noValidate>
        ...
        <label><span>Email</span><input type="email" value={email} onChange={(e) => setEmail(e.target.value)} readOnly={Boolean(invitedEmail)} required maxLength={254} autoComplete="email" style={{ width: '100%', padding: '0.65rem', marginTop: 4 }} />{email && fieldErrors.email && <small className={styles.error}>{fieldErrors.email}</small>}</label>
