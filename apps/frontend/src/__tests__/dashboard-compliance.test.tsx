...
jest.mock('../hooks/useFinancials', () => ({ useFinancials: () => ({ revenue: [], isLoading: false }) }));
jest.mock('../hooks/useForecasting', () => ({ useForecasting: () => ({ forecast: [], isLoading: false }) }));
...
jest.mock('../hooks/useCompliance', () => ({
  useCompliance: () => ({ isLoading: false, error: null, items: mockComplianceItems }),
}));
